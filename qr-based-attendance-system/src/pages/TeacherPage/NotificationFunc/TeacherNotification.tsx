...
interface Notification {
  ...
  is_read: boolean;
  created_at: string;
  room_id?: string; // Optional field for redirection
}

const API_URL = import.meta.env.VITE_API_URL;

const TeacherNotification: React.FC = () => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.post(
          `${API_URL}/notifications/get-teacher-notifications`,
          {},
          {
            headers: {
  ...
  const handleNotificationClick = async (notification: Notification) => {
    try {
      ...
      // Redirect if the notification has a room_id
      if (notification.room_id) {
        navigate(`/teacher-dashboard/rooms/${notification.room_id}`);
      }
    } catch (err) {
  ...
  return (
    <div className="notification-container">
      <h1 className="notification-title">Teacher Notifications</h1>
